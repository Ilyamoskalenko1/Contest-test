  ...
  - navigation [ref=e3]:
    - generic [ref=e4]:
      ...
      - list [ref=e8]:
        ...
        - listitem [ref=e13]:
          - button "Riley94" [ref=e14] [cursor=pointer]:
            - text: Riley94
            - img [ref=e15]
  - generic [ref=e16]:
    - generic [ref=e17]:
      ...
      - generic [ref=e19]:
        ...
        - textbox "Name" [ref=e27]: Riley94
        - generic [ref=e28]: Email
        - textbox "Email" [ref=e30]: test_1776263522845@mailinator.com
        - generic [ref=e32]:
          - img [ref=e35]
...
```ts
  ...
  66 | 
  67 |   // 1. Открываем дропдаун с именем пользователя
  68 |   await page.locator('#navbarScrollingDropdown').click();
  69 |   await page.waitForTimeout(800);
  70 | 
  71 |   // 2. Кликаем Settings (открывает offcanvas)
  72 |   await page.locator('button.dropdown-item[data-bs-toggle="offcanvas"]').click();
  73 |   await page.waitForTimeout(1500);
  74 | 
  75 |   // 3. Кликаем Profile Settings
  76 |   await page.locator('.offcanvas a:has-text("Profile Settings")').click();
> 77 |   await page.waitForURL(/\/users\/\d+\/edit/, { timeout: 30000 });
     |              ^ TimeoutError: page.waitForURL: Timeout 30000ms exceeded.
  78 |   await page.waitForTimeout(1500);
  79 | 
  80 |   // 4. Загружаем фото (input скрыт, setInputFiles работает с hidden-инпутами)
  81 |   await page.locator('#user_avatar').setInputFiles('/Users/aleksandrpasinin/Documents/Партиципуки/аватар.png');
  82 |   await page.waitForTimeout(1000);
  83 | 
  84 |   // 5. Выбираем регион
  85 |   await page.locator('#user_region_id').selectOption({ index: 1 });
  86 |   await page.waitForTimeout(500);
  87 | 
  88 |   // 6. Вводим локацию
  89 |   await page.locator('#user_location').click();
  90 |   await page.locator('#user_location').pressSequentially('New York', { delay: 50 });
  91 | 
  92 |   // 7. Нажимаем Save Changes (первая кнопка submit — главная форма профиля)
  93 |   await page.locator('input[type="submit"][name="commit"]').first().click();
  94 |   await page.waitForTimeout(3000);
  95 | 
  96 |   // Пауза — браузер остаётся открытым для проверки
  97 |   await page.pause();
  98 | });
  99 | 
```